...
def get_db_path(entity):
    ...
    if entity == 'doid':
        ...
        return config.path_db_hp
    elif entity == 'go':
        return config.path_db_go
    return ''
...
def loading_items(is_chebi=False, is_doid=False, is_go=False, is_hp=False):
    ...
    # if go.owl does not exist
    if is_go and not os.path.exists(get_owl_path('go')):
        url = "http://purl.obolibrary.org/obo/go.owl"
        get_owl(url, path=os.path.dirname(get_owl_path('go')))

    # if hp.owl does not exist
    if is_hp and not os.path.exists(get_owl_path('hp')):
