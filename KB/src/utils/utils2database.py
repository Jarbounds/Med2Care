...
def create_structural_table(tablename):
    """
...
def get_similar(table_name, quart, sim):
    """
    Get the item 1, item 2 of the 1st quartile in the similarity db
    :param table_name: name of the table saved in mysql
    :param quart: quartile
    ...
    try:
        ...
        # Prepare SQL query to read a record into the database.        
        sql = f"SELECT comp_1, comp_2 FROM {table_name} where {sim} >= ( \
                    SELECT {sim} \
                    FROM ( SELECT s.*,  @row_num :=@row_num + 1 AS row_num \
                            FROM {table_name} s, (SELECT @row_num:=0) counter \
