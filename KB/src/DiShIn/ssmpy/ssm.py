...
def ssm_lin(entry1, entry2):
    ...
    if aux > 0:
        return abs(2 * shared_ic(entry1, entry2) / aux)
    else:
        return 1.0
...
def ssm_rel(entry1, entry2):
    """Calculate Schlicker’s semantic similarity.
    ...
    :Example:
        ...
        >>> gold = ssmpy.get_id("gold")
        >>> silver = ssmpy.get_id("silver")
        >>> ssmpy.ssm_rel(gold, silver)
        
    """

    return ssm_lin(entry1, entry2) * ( 1 - 10**(-shared_ic(entry1, entry2)))


def ssm_jac(entry1, entry2):
    """Calculate JACCARD's semantic similarity.
    ...
    distance = (
        information_content(entry1)
        + information_content(entry2)
        - shared_ic(entry1, entry2)
    )
    
    return shared_ic(entry1, entry2)/distance


def ssm_iclch(entry1, entry2):
    """Calculate Sánchez and Batet's semantic similarity.
    ...
    num = information_content(entry1) + information_content(entry2) - 2*shared_ic(entry1, entry2) + 1
    den = 2*max(information_content(entry1),information_content(entry2))
...
def fast_rel_jac_islch(all_ancestors, df_entry_ancestors, df_entry_ic, it1, it2):
    """
     Calculates the SCHLICKER, JACCARD, SÁNCHEZ and BATET MICA INTRINSIC similarity between it1 and it2
    ...
    if it1 == it2:
        ...
        sim_rel = sim_lin * (1 - 10**(-ic_ancestors_max))  

        sim_jac = ic_ancestors_max / distance

        distance = df_entry_ic[df_entry_ic.id == it1].IC.to_list()[0] + df_entry_ic[df_entry_ic.id == it2].IC.to_list()[
            0] - 2*ic_ancestors_max + 1

        sim_islch = - math.log10( distance / (2*max( df_entry_ic[df_entry_ic.id == it1].IC.to_list()[0],
                df_entry_ic[df_entry_ic.id == it2].IC.to_list()[0] )))
...
def new_light_similarity(conn, entry_ids_1, entry_ids_2, metric, cpu_cores):
    """
    main function
    :param conn: db_connection
    :param entry_ids_1: list of entries 1
    ...
    if metric == 'rel':
        ...
        for it1 in ids_list_1:
            pool = mp.Pool(cpu_cores)
            results.append(pool.starmap(fast_rel,
                                        [(all_ancestors, df_entry_ancestors, df_entry_ic, it1, it2) for it2 in
                                            ids_list_2]))
        ...
        for it1 in ids_list_1:
            pool = mp.Pool(cpu_cores)
            results.append(pool.starmap(fast_islch,
                                        [(all_ancestors, df_entry_ancestors, df_entry_ic, it1, it2) for it2 in
                                            ids_list_2]))
         ...
         for it1 in ids_list_1:
            pool = mp.Pool(cpu_cores)
            results.append(pool.starmap(fast_jac,
                                        [(all_ancestors, df_entry_ancestors, df_entry_ic, it1, it2) for it2 in
                                            ids_list_2]))
        ...
        for it1 in ids_list_1:
            pool = mp.Pool(cpu_cores)
            results.append(pool.starmap(fast_rel_jac_islch,
                                        [(all_ancestors, df_entry_ancestors, df_entry_ic, it1, it2) for it2 in
                                            ids_list_2]))
